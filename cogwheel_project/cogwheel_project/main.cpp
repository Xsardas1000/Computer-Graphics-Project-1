...
uint allocation_of_connectivity(Matrix<uint> &binaryMap) {
    ...
    Matrix<uint> labels(1, binaryMap.n_rows * binaryMap.n_cols);
    map<uint, uint> components_labels;
    
    for (uint i = 0; i < binaryMap.n_rows * binaryMap.n_cols; i++) {
        labels(0, i) = 0;
    }
    ...
    uint label = 1;
    uint first = 1;
    uint changes_found = 5;
    while (changes_found) {
        //проход сверху вниз, слева направо
        if (k % 2) {
            for (uint i = 0; i < binaryMap.n_rows - 1; i++) {
                for (uint j = 0; j < binaryMap.n_cols - 1; j++) {
                    if (binaryMap(i + 1, j + 1) == 0) {
                        continue;
                    } else if (binaryMap(i, j + 1) == 0 && binaryMap(i + 1, j) == 0) {
                        if (first) {
                            ++label;
                            binaryMap(i + 1, j + 1) = label;
                            labels(0, label) = label;
                        }
                    } else if (binaryMap(i + 1, j) != 0 && binaryMap(i, j + 1) == 0) {
                        
                        if (first) {
                            binaryMap(i + 1, j + 1) = labels(0, binaryMap(i + 1, j));
                        }
                    } else if (binaryMap(i + 1, j) == 0 && binaryMap(i, j + 1) != 0) {
                        
                        if (first) {
                            binaryMap(i + 1, j + 1) = labels(0, binaryMap(i, j + 1));
                        }
                    } else if (binaryMap(i + 1, j) != 0 && binaryMap(i, j + 1) != 0) {
                        
                        if (labels(0, binaryMap(i, j + 1)) == labels(0, binaryMap(i + 1, j))) {
                            binaryMap(i + 1, j + 1) = labels(0, binaryMap(i, j + 1));
                            
                        } else {
                            if (labels(0, binaryMap(i, j + 1)) < labels(0, binaryMap(i + 1, j))) {
                                labels(0, binaryMap(i + 1, j)) = labels(0, binaryMap(i, j + 1));
                                binaryMap(i + 1, j + 1) = labels(0, binaryMap(i, j + 1));
                            } else {
                                labels(0, binaryMap(i, j + 1)) = labels(0, binaryMap(i + 1, j));
                                binaryMap(i + 1, j + 1) = labels(0, binaryMap(i + 1, j));
                            }
                        }
                    }
            ...
            //обратный проход
            for (uint i = binaryMap.n_rows - 1; i > 0; i--) {
                for (uint j = binaryMap.n_cols - 1; j > 0; j--) {
                    if (binaryMap(i - 1,j - 1) == 0) {
                        continue;
                    } else if (binaryMap(i - 1, j) != 0 && binaryMap(i, j - 1) == 0) {
                        binaryMap(i - 1, j - 1) = labels(0, binaryMap(i - 1, j));
                    } else if (binaryMap(i - 1, j) == 0 && binaryMap(i, j - 1) != 0) {
                        binaryMap(i - 1, j - 1) = binaryMap(i, j - 1);
                    } else if (binaryMap(i, j - 1) != 0 && binaryMap(i - 1, j) != 0) {
                        
                        if (labels(0, binaryMap(i - 1, j)) == labels(0, binaryMap(i, j - 1))) {
                            binaryMap(i - 1, j - 1) = labels(0, binaryMap(i, j - 1));
    ...
    uint components_number = 0;
    //подсчет связных областей
    for (uint i = 2; i < binaryMap.n_rows * binaryMap.n_cols; ++i){
        if (labels(0, i) == i){
            components_labels[i] = ++components_number - 1;
        }
    }
    ...
    for (uint i = 0; i < binaryMap.n_rows; i++) {
        for (uint j = 0; j < binaryMap.n_cols; j++) {
            if (binaryMap(i, j)) {
                binaryMap(i, j) = components_labels[binaryMap(i, j)] + 1;
            }
        }
    }
...
int gear_cloves_counter_and_broken_gear_finder(vector<Component>& components_array, const Matrix<uint> &binaryMap, int f, Image &image) {
    ...
    for (uint k = 0; k < components_array.size(); k++) {
        ...
        for (uint i = 0; i < components_array[k].pixels_array.size(); i++) {
            uint x = components_array[k].pixels_array[i].a1;
            uint y = components_array[k].pixels_array[i].a2;
            tmp_binaryMap(x, y) = 1;
        }
        ...
        /*
         for (uint i = 0; i < binaryMap.n_rows; i++) {
         for (uint j = 0; j < binaryMap.n_cols; j++) {
         if (tmp_binaryMap(i, j)) {
         image(i, j) = make_tuple(255, 0,0);
         }
         else {
         image(i, j) = make_tuple(0, 0,0);
         }
         }
         }
...
repair_mechanism(const Image& in, string path)
{
    ...
    if (idx_axle >= 0) {
        ...
        result_idx = gear_cloves_counter_and_broken_gear_finder(components_array, binaryMap, base, result_image);
        ...
        broken_gear_idx = gear_cloves_counter_and_broken_gear_finder(components_array, binaryMap, base, result_image);
